...
class help_function:
    ...
    def get_data(self, folder_path, image_type, last_throw = False ):
    ...
    def get_next_batch(self, data,  i,size_batch = 200):


        if size_batch < 1:
            raise ValueError('The size_batch should be greater than 0. The value of size_batch was: {}'.format(size_batch))
        if len(data) < i*size_batch:
            return None
        batch = data[(i-1)*size_batch: i*size_batch]

        return batch

    def extract_details_from_file_name_two_color(self, data):
        new_data = []
        for (i,img) in enumerate(data):
            try:
                ...
                details[7] = int(age_gender_color_by_regex.group(4).replace("_", ""))
                details[num_of_color + 2] = int(1)
                new_data.append(np.array([details, img[1]]))
            except:
                continue
            # img_name = int(age_gender_color_by_regex.group(4).replace("_", ""));
